...
|------------|-----------|-------------|
| FAISS HNSW | 7.18 MB   | 1× (baseline)|
| AXIOM Map  | 40 KB     | **29.4×** faster |
...
| W2 (proj_up)            | r → k × d_model          | 512 → 64 × 768 ✅       |
| LayerNorm               | Post-projection           | ✅                       |
...
2. **Aggressive safety** — the θ=0.35 threshold with 150-entity item
...
1. **Distiller**: 99% accuracy, 0.995 F1, 100% hallucination catch ✅
2. **Latent Priming**: Full W1/W2 → virtual token → embedding injection architecture ✅
3. **Safety Governor**: 100% catch rate, token-level filtering operative ✅
4. **Compression**: 179× smaller than FAISS ✅
5. **Speed**: 29.4× faster than FAISS ✅
6. **Offline**: Entire pipeline runs without network access ✅

The remaining gap between POC and production is **scale** (GPT-2 124M
vs Llama 3B, 150 vs 10K+ facts, 200 vs 50K+ training QA pairs), not
architecture. The paper's core innovation — replacing vector databases
with HDC Axiom Maps for grounded SLM generation — is proven viable.
